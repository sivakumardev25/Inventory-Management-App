...
export default function Inventory() {
  ...
  useEffect(() => {
    api
      .get("/clients", { params: { active: true } })
  ...
  const openAdd = () => {
    ...
    setForm({
      client: "",
      date: new Date().toISOString().slice(0, 10),
      lines: [{ product: "", quantity: "", priceAtTime: "" }],
      notes: "",
    });
    setModal(true);
  };
  ...
  const addLine = () =>
    setForm((p) => ({
      ...p,
      lines: [...p.lines, { product: "", quantity: "", priceAtTime: "" }],
    }));
  const removeLine = (i) =>
  ...
  const save = async (e) => {
    ...
    for (const line of form.lines) {
      if (!line.product) return toast.error("Please select a product.");
      ...
      const payload = {
        ...
        lines: form.lines
          ...
          .map((l) => ({
            ...
            priceAtTime: parseFloat(l.priceAtTime) || undefined,
          })),
      };
      if (editing) {
  ...
  const remove = async (e) => {
    ...
    try {
      await api.delete(`/inventory/${e._id}`);
  ...
  const filtered = entries.filter((e) => {
    ...
    return (
      e.client?.name?.toLowerCase().includes(q) || e.client?.phone?.includes(q)
    );
  });
                    ...
                    <tr key={e._id}>
                      ...
                      <td>
                        {e.lines.map((l, i) => (
                          ...
                          >
                            <strong>{l.product?.name}</strong> — {l.quantity} ×
                            ₹{l.priceAtTime} ={" "}
                      ...
                      <td>
                        <strong
                          style={{ color: "var(--navy)", fontSize: ".9rem" }}
                        >
                          ₹
                    ...
                    <div key={i} className="item-row">
                      ...
                      <div className="field" style={{ margin: 0 }}>
                        <input
                          ...
                          min="0.0"
                          step="0.5"
                          value={line.priceAtTime}
                          onChange={(e) =>
                            updateLine(i, "priceAtTime", e.target.value)
                          }
                        />
                      </div>
                      <div
                      ...
                      >
                        ...
                        >
                          {line.quantity && line.priceAtTime
